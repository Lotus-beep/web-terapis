    ...
    <div class="row g-4">
        ...
        <div class="col-md-6 col-lg-4">
            <div class="service-card-new">
                {{-- Category badge --}}
                <div class="cat-badge">{{ $service->category_label }}</div>
                ...
                {{-- Body --}}
                <div class="svc-body">
                    <h5 class="svc-title">{{ $service->name_service }}</h5>
                    @if($service->description)
                        <p class="svc-desc">{{ Str::limit($service->description, 90) }}</p>
                    @endif

                    <div class="svc-meta">
                        <div><i class="bi bi-person-badge-fill"></i> {{ $service->terapis->username }}</div>
                        @if($service->terapis->rating > 0)
                        <div><i class="bi bi-star-fill" style="color:var(--yellow-main);"></i> {{ number_format($service->terapis->rating, 1) }}</div>
                        @endif
                    </div>

                    <div class="svc-footer">
                        <div class="svc-price">Rp {{ number_format($service->price, 0, ',', '.') }}</div>
...
<style>
    ...
    .svc-meta i { color: var(--green-mid); }
