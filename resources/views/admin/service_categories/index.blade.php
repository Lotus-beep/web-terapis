                ...
                <div class="card h-100" style="border-radius:14px;overflow:hidden;transition:transform .2s,box-shadow .2s;"
                    ...
                    <div style="height:140px;overflow:hidden;position:relative;">
                        ...
                        <div style="position:absolute;bottom:10px;left:14px;display:flex;align-items:center;gap:8px;">
                            <span
                                style="width:32px;height:32px;background:rgba(255,255,255,.18);border-radius:8px;display:flex;align-items:center;justify-content:center;">
                        ...
                        @if(!$cat->is_active)
                            <span
                                style="position:absolute;top:10px;right:10px;background:rgba(192,57,43,.85);color:white;font-size:.68rem;font-weight:700;padding:2px 8px;border-radius:50px;">
                                Nonaktif
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="card-body p-3">
                        @if($cat->header_content)
                            <p class="text-muted mb-2" style="font-size:.85rem;line-height:1.5;font-weight:bold;">
                                {{ Str::limit($cat->header_content, 80) }}
                            </p>
                        @endif
                        @if($cat->description)
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <div style="font-size:.75rem;color:var(--text-muted);">
                                ...
                                {{ $cat->bookings()->count() }} booking
                            </div>
                            <div style="font-size:.8rem;font-weight:700;color:var(--green-dark);">
                                Rp {{ number_format($cat->price, 0, ',', '.') }}
                            </div>
                            <div class="d-flex gap-1">
                                <a href="{{ route('admin.service-categories.edit', $cat->id) }}" class="btn btn-sm btn-warning"
